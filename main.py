...
class Button:
    def __init__(self, x, y, image):
        ...
        self.rect = self.img.get_frect(topleft=(x, y))
        self.hover_rect = self.hover_img.get_frect(center=self.rect.center)
...
while run:
    ...
    render_surface.blit(bg_long, (bg_long_scroll + 1280, 0))
    ...
    if game_state == STATE_PLAYING:
        ...
        for p in pipe_group:
            if not p.scored and flappy.rect.left > p.rect.right and p.position == -1:
                ...
                score_rect, score_scale = score_surface.get_frect(
    ...
    if game_state != STATE_MENU:
        if score_scale > 1.0:
            ...
            render_surface.blit(s, s.get_frect(center=(SCREEN_WIDTH//2, 50)))
        else:
